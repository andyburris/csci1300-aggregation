...
export function searchBooks(term: string): Promise<Book[]> {
    ...
    return fetch(`https://openlibrary.org/search.json?title=${term}`)
        ...
        .then((data) => data.docs.map((book: any) => new Book(book.key, book.title, book.author_name?.join(", ") || "Unknown", book.cover_i ? `https://covers.openlibrary.org/b/id/${book.cover_i}-M.jpg` : null)))
}
